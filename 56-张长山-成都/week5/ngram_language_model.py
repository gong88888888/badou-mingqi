...
class NgramLanguageModel:
    def __init__(self, corpus=None, n=3):
        ...
        self.ngram_count_dict = dict(
            (x + 1, defaultdict(int)) for x in range(n)
        )  # 统计词频（window_size=1,2,3）
        self.ngram_count_prob_dict = dict(
            (x + 1, defaultdict(int)) for x in range(n)
        )  # 统计概率（单字、双字、三字词的概率）
        ...
        for window_size in range(1, self.n + 1):
            for ngram, count in self.ngram_count_dict[window_size].items():
                if window_size > 1:
                    ...
                    ngram_prefix_count = self.ngram_count_dict[window_size - 1][
                        ngram_prefix
                    ]  # Count(a,b)
                else:
                ...
                self.ngram_count_prob_dict[window_size][ngram] = (
                    count / ngram_prefix_count
                )
        return
...
if __name__ == "__main__":
    ...
    print(lm.predict("萦绕在世界经济的阴霾仍未消退"))
